...
const STATE_KEY = "translator-state-v1";

const languageNames = Object.fromEntries(LANGUAGES);
let history = loadHistory();

function optionMarkup(code, label) {
...
function populateLanguageSelects() {
  for (const [code, label] of LANGUAGES) {
    els.sourceLang.appendChild(optionMarkup(code, label));
    els.targetLang.appendChild(optionMarkup(code, label));
  }
  ...
  if (!history.length) {
    els.historyList.innerHTML = `
      <div class="history-item">
        <p class="placeholder">还没有历史记录。翻译一次后这里就会自动出现。</p>
      </div>
    `;
    renderCounts();
    return;
...
async function translate() {
  ...
  try {
    ...
    setStatus(error instanceof Error ? error.message : "翻译失败。", "danger");
  } finally {
...
function init() {
  ...
  wireEvents();
  renderHistory();
  setOutput(els.sourceText.value ? "" : "");
  setStatus("准备就绪。输入文本后点击翻译即可。");
  renderCounts();
}
